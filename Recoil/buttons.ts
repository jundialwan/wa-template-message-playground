...
export const allQuickReplyButtonSelector = selector<
...
export const quickReplyButtonSelector = selectorFamily<
  QuickReplyButtonComponent,
...
>({
  ...
  get:
    ...
    ({ get }) => {
      ...
      return (
        buttonsComponent.reply?.[order] || defaultValueQuickReplyButtons[2]
      );
    },
  set:
    (order) =>
    ({ set, get }, quickReplyButton) => {
      ...
      if (!(quickReplyButton instanceof DefaultValue)) {
        ...
        if (selectedButton) {
          set(buttonsAtom, (prev) => {
            let newReplyButtonState = prev.reply
              ? [...prev.reply]
              : [...defaultValueQuickReplyButtons];
            newReplyButtonState[order] = {
              ...quickReplyButton,
              text: quickReplyButton.text.substring(0, 20),
            };
...
export const ctaButtonSelector = selectorFamily<
  CTAButtonComponent,
  CTAButtonIndex
>({
  ...
  set:
    ...
    ({ set, get }, ctaButton) => {
      ...
      if (!(ctaButton instanceof DefaultValue)) {
        ...
        if (selectedButton) {
          set(buttonsAtom, (prev) => {
            let newCTAButtonState = prev.cta
              ? [...prev.cta]
              : [...defaultValueCTAButtons];
            newCTAButtonState[order] = {
              ...ctaButton,
              text: ctaButton.text.substring(0, 20),
            };
