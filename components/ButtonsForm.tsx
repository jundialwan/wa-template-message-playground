...
const CTAButtonInput: FC<{ order: CTAButtonIndex }> = ({ order }) => {
  ...
  const onCTAContextChange = (e: any) => setThisButton(curr => ({...curr, [thisButton.type === 'call-phone' ? 'phone' : 'url']: e.target.value}))
      ...
      <div className="gap-1">
        ...
        <div className="flex flex-row gap-1 items-center border px-3 py-2 rounded-lg">
          ...
          <div className="flex flex-col">
            ...
            <input
              type="text" name={`cta-text-${order}`} id={`cta-text-${order}`} value={thisButton.text} onChange={onButtonTextChange}
          ...
          <div className="flex flex-col">
            <div className="flex flex-row items-center gap-1 font-semibold">
              ...
              <span className="text-xs">({thisButton.type === 'call-phone' ? thisButton.phone?.length || 0 : thisButton.url?.length || 0}/20)</span>
            </div>

            <input
              type="text" name={`cta-context-${order}`} id={`cta-context-${order}`} value={thisButton.type === 'call-phone' ? thisButton.phone : thisButton.url} onChange={onCTAContextChange}
              className="border py-1 px-2 rounded text-black text-sm focus:outline-none focus:ring-2 focus:ring-green-100 focus:border-green-600"
            />
          </div>
        </div>
