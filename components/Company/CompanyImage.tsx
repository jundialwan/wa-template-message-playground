...
const CompanyImage: FC = () => {
  ...
  // console.log('companyPathImage?.length > 0 ', companyPathImage?.length > 0);
  return (
    <div tw='mt-2 flex flex-col items-start'>
      ...
      {companyPathImage?.length > 1 ? (
        <div tw='flex w-full items-start mt-4'>
          <div tw='w-24'>
            <AspectRatio tw='max-w-[400px]' ratio={1 / 1}>
              <img src={companyPathImage} alt='Image Bot' tw='w-full h-full' />
            </AspectRatio>
          </div>
          <button type='button' onClick={() => setCompanyImage('')} tw=' flex justify-center items-center  bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 focus:ring-offset-blue-200 text-white transition-all text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  w-8 h-8 rounded-lg '>
      ...
      <label tw='block'>
        <input type='text' tw='mt-1 block w-full' placeholder='path image' onChange={(e) => setCompanyImage(e.target.value)} value={companyPathImage} />
      </label>
    </div>
  );
};
