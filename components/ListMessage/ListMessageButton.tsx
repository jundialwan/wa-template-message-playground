      ...
      <Transition appear show={isShowMessage && isOpened}>
        <Transition.Child enter='ease-out duration-300' enterFrom='opacity-0 scale-95' enterTo='opacity-100 scale-100' leave='ease-in duration-200' leaveFrom='opacity-100 scale-100' leaveTo='opacity-0 scale-95' as='div' tw='bg-white rounded-tr-2xl rounded-tl-2xl w-full absolute max-h-[450px] left-0 right-0 z-20 px-[18px] py-[23px] bottom-0'>
          <div tw='flex flex-col h-full'>
            ...
            <div tw='flex flex-col mt-auto space-y-5'>
              {message.listMessage?.map((data, index) => {
                if (data.enabled) {
                  return (
                    <div>
                      <label tw='inline-flex items-center w-full' key={index}>
                        <span tw='text-sm text-black'>{data.title}</span>
                        <InputRadio type='radio' name='accountType' value='personal' />
                      </label>
                      <span tw='text-xs text-gray-400 mt-1'>{data?.subtitle}</span>
                    </div>
                  );
                }
              })}
            </div>
            <button tw='bg-[#00BF56] rounded-md w-full h-[46px] flex items-center justify-center text-white mt-8' onClick={() => setShowMessage(!isShowMessage)}>
              KIRIM
            </button>
          </div>
        </Transition.Child>
