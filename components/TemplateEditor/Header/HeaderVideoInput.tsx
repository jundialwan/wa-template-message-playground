    ...
    <div className='mt-2 flex flex-col items-start'>
      ...
      {headerVideoPath?.length > 1 ? (
        ...
        <label htmlFor='upload-file' className='py-2 mt-1 px-4 flex justify-center items-center  bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 focus:ring-offset-blue-200 text-white  transition-all text-center text-sm font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg cursor-pointer'>
          ...
          Upload
          <input accept='video/*' id='upload-file' type='file' onChange={(e) => handleHeaderVideoUploadChange(e, messageId)} className='hidden' />
        </label>
      )}
      <label className='block'>
        <input type='text' className='mt-1 block w-full' placeholder='path video' onChange={(e) => handleHeaderVideoChange(e, messageId)} value={headerPathVideo} />
      </label>
    </div>
  );
};
